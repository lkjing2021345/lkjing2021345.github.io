...
// ============================================================
// 图片注解识别：以"图 N:"开头的标题视为图片注解，去掉左侧黑条
// ============================================================
(() => {
    document.querySelectorAll('.article-content h1, .article-content h2, .article-content h3, .article-content h4, .article-content h5').forEach(h => {
        if (/^图\s*[0-9０-９]/.test(h.textContent.trim())) {
            h.classList.add('caption-heading');
        }
    });
})();
